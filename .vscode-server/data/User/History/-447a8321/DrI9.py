...
class Maze:
    ...
    def _solve_r(self,i,j):
        ...
        for direction in range(0,4):
            if direction == 0 and i-1 >= 0 and self._cells[i][j].top_wall == False and self._cells[i-1][j].visited == False:
                self._cells[i][j].draw_move(self._cells[i-1][j])
                return self._solve_r(i-1,j)
            else:
                self._cells[i][j].draw_move(self._cells[i-1][j],True)
            if direction == 3 and j-1 >= 0 and self._cells[i][j].left_wall == False and self._cells[i][j-1].visited == False:
                self._cells[i][j].draw_move(self._cells[i][j-1])
                return self._solve_r(i,j-1)
            else:
                self._cells[i][j].draw_move(self._cells[i][j-1],True)
            if direction == 1 and i+1 < self.num_cols and self._cells[i][j].right_wall == False and self._cells[i+1][j].visited == False:
                self._cells[i][j].draw_move(self._cells[i+1][j])
                return self._solve_r(i+1,j)
            else:
                self._cells[i][j].draw_move(self._cells[i+1][j],True)
            if direction == 2 and j+1 < self.num_rows and self._cells[i][j].bottom_wall == False and self._cells[i][j+1].visited == False:
                self._cells[i][j].draw_move(self._cells[i][j+1])
                return self._solve_r(i,j+1)
            else:
                self._cells[i][j].draw_move(self._cells[i][j+1],True)
