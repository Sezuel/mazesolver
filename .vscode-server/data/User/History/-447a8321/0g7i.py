...
class Maze:
    ...
    def _solve_r(self,i,j):
        ...
        if i == self.num_cols-1 and j == self.num_rows-1:
            return True
        if i-1 >= 0 and self._cells[i][j].top_wall == False and self._cells[i-1][j].visited == False:
            self._cells[i][j].draw_move(self._cells[i-1][j])
            return self._solve_r(i-1,j)
        else:
            self._cells[i][j].draw_move(self._cells[i-1][j],True)
        if j-1 >= 0 and self._cells[i][j].left_wall == False and self._cells[i][j-1].visited == False:
            self._cells[i][j].draw_move(self._cells[i][j-1])
            return self._solve_r(i,j-1)
        else:
            self._cells[i][j].draw_move(self._cells[i][j-1],True)
        if i+1 < self.num_cols and self._cells[i][j].right_wall == False and self._cells[i+1][j].visited == False:
            self._cells[i][j].draw_move(self._cells[i+1][j])
            return self._solve_r(i+1,j)
        else:
            self._cells[i][j].draw_move(self._cells[i+1][j],True)
        if j+1 < self.num_rows and self._cells[i][j].bottom_wall == False and self._cells[i][j+1].visited == False:
            self._cells[i][j].draw_move(self._cells[i][j+1])
            return self._solve_r(i,j+1)
        else:
            self._cells[i][j].draw_move(self._cells[i][j+1],True)
